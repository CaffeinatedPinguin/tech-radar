...
const link = z.object({label: nonEmpty, url: z.string().url()});
...
const alternative = z.object({name: nonEmpty, reason: nonEmpty});
const adr = z.object({
  ...
  consequences: z.object({positive: z.array(nonEmpty).default([]), negative: z.array(nonEmpty).default([])}),
...
const radar = z.object({
  ...
  title: nonEmpty,
  areas: z.array(z.object({id: z.enum(areas), name: nonEmpty})).length(4),
});
...
async function yamlFiles(directory) {
  ...
  return entries.filter((entry) => entry.isFile() && entry.name.endsWith('.yaml')).sort((a, b) => a.name.localeCompare(b.name));
}
...
async function readEntityDirectory(kind, schema) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      throw new Error(`${source}:document invalid YAML (${error instanceof Error ? error.message : String(error)})`, {cause: error});
    }
    const result = schema.safeParse(parsed);
    if (!result.success) {
      throw new Error(result.error.issues.map((issue) => formatIssue(source, issue)).join('\n'));
    }
...
function assertReference(source, path, id, index, label) {
  if (!index.has(id)) throw new Error(`${source}:${path} references unknown ${label} "${id}"`);
}
...
function detectCycles(graph, sourceFor) {
  ...
  const walk = (node, path) => {
    if (visiting.has(node)) throw new Error(`${sourceFor(node)}:${path} creates a relationship cycle at "${node}"`);
  ...
  for (const node of graph.keys()) walk(node, 'relationships');
}
...
function derive(technologies, adrs) {
  ...
  for (const item of technologies) {
    ...
    data.history.forEach((entry, index) => {
      if (entry.adr) assertReference(item.source, `history[${index}].adr`, entry.adr, adrById, 'ADR');
    });
  }
  for (const item of adrs) {
    ...
    if (data.supersedes) {
      if (data.supersedes === data.id) throw new Error(`${item.source}:supersedes cannot reference itself`);
  ...
  technologies.forEach((item) => item.data.replaces.forEach((id) => replacedBy.get(id).push(item.data.id)));
  const supersededBy = new Map(adrs.map((item) => [item.data.id, undefined]));
  adrs.forEach((item) => {
    if (item.data.supersedes) supersededBy.set(item.data.supersedes, item.data.id);
  });
  const adrsByTechnology = new Map(technologies.map((item) => [item.data.id, []]));
  adrs.forEach((item) => item.data.technologies.forEach((id) => adrsByTechnology.get(id).push(item.data.id)));
...
async function main() {
  ...
  const radarResult = radar.safeParse(YAML.parse(await readFile(radarPath, 'utf8')));
  if (!radarResult.success) throw new Error(radarResult.error.issues.map((issue) => formatIssue(radarSource, issue)).join('\n'));
  const ids = radarResult.data.areas.map((area) => area.id);
  if (new Set(ids).size !== 4 || !areas.every((area) => ids.includes(area))) throw new Error(`${radarSource}:areas must contain exactly development, platform, data and security`);
  ...
  for (const group of [technologies, adrs]) {
    for (const item of group) {
      if (allIds.has(item.data.id)) throw new Error(`${item.source}:id duplicates ${allIds.get(item.data.id)}`);
