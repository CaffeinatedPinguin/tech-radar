...
function RadarSvg({technologies, onSelect}: {technologies: Technology[]; onSelect: (technology: Technology) => void}) {
  ...
  return (
    <svg className="radar-svg" viewBox="0 0 800 700" role="img" aria-label="Technology radar with four rings and four areas">
      <g className="radar-rings">
        {RINGS.map((ring) => (
          <g key={ring}>
            <circle cx="400" cy="350" r={ringRadius[ring]} />
            <text x="400" y={350 - ringRadius[ring] + 18}>{ring.toUpperCase()}</text>
          </g>
        ))}
      </g>
      <line className="radar-axis" x1="400" y1="36" x2="400" y2="664" />
      <line className="radar-axis" x1="86" y1="350" x2="714" y2="350" />
      <text className="radar-area-label" x="115" y="64">DEVELOPMENT</text>
      <text className="radar-area-label" x="562" y="64">PLATFORM</text>
      <text className="radar-area-label" x="568" y="650">DATA</text>
      <text className="radar-area-label" x="112" y="650">SECURITY</text>
      {points.map((point) => (
        <g
          ...
          tabIndex={0}
          role="button"
          aria-label={`${point.name}, ${point.ring}, ${point.usage}`}
        ...
        >
          ...
          <text x={point.x} y={point.y + 4}>{point.name}</text>
        </g>
      ))}
    </svg>
  );
  ...
  return (
    <>
      <PageHeader
        ...
        description="A deterministic map of what we use, what we are trying, and what deserves a closer look."
        action={<span className="count-label"><strong>{technologies.length}</strong> technologies</span>}
      />
      <TechnologyFilters filters={filters} />
      <div className="mobile-view-toggle">
        <button className={showRadar ? 'button active' : 'button quiet'} onClick={() => setShowRadar(true)} type="button">Show radar</button>
        <button className={!showRadar ? 'button active' : 'button quiet'} onClick={() => setShowRadar(false)} type="button">Show list</button>
      </div>
      <div className="radar-layout">
        <section className={showRadar ? 'surface radar-surface' : 'surface radar-surface hide-on-mobile'}>
          <RadarSvg technologies={technologies} onSelect={(technology) => navigate(`/technologies/${technology.id}`)} />
        </section>
        <section className="surface radar-list">
          <div className="section-heading">
            <div>
              <span className="eyebrow">ACCESSIBLE INDEX</span>
              <h2>Technology list</h2>
            </div>
            <span className="muted">{technologies.length} shown</span>
          </div>
          {technologies.length ? (
            <div className="compact-list">
              {sortedTechnologies.map((technology) => (
                <Link key={technology.id} to={`/technologies/${technology.id}`}>
                  <span>
            ...
            <EmptyState title="No matches" message="Clear the filters to restore the radar." />
          )}
        </section>
      </div>
    </>
