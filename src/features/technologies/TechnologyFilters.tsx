...
export function TechnologyFilters({filters}: {filters: TechnologyFilters}) {
  ...
  return (
    <div className="filters-panel" aria-label="Technology filters">
      ...
      <div className="filter-selects">
        <label>
          ...
          <select value={filters.area} onChange={(event) => update('area', event.target.value)}>
            ...
            {AREAS.map((area) => <option key={area} value={area}>{labelCase(area)}</option>)}
          </select>
        </label>
        <label>
          Ring
          <select value={filters.ring} onChange={(event) => update('ring', event.target.value)}>
            <option value="all">All rings</option>
            {RINGS.map((ring) => <option key={ring} value={ring}>{labelCase(ring)}</option>)}
          </select>
        </label>
        <label>
          Usage
          <select value={filters.usage} onChange={(event) => update('usage', event.target.value)}>
            <option value="all">All usage</option>
            {USAGES.map((usage) => <option key={usage} value={usage}>{labelCase(usage)}</option>)}
          </select>
        </label>
      </div>
      {hasFilters ? (
        <button className="button quiet" type="button" onClick={clear}>
          <IconX size={16} />
          Clear filters
        </button>
      ) : (
        <span className="filter-hint">
          ...
          Filters are shareable in the URL
        </span>
      )}
    </div>
  );
